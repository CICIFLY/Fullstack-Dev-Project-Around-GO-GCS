...
func getPostFromSearchResult(searchResult *elastic.SearchResult) []Post {
	var ptype Post
	var posts []Post
	// sanity check if it is the expected type . NoSQL data type is flexible so it is necessary to have a sanity check
	// iterate each item to do the sanity check
	// “_” -----> placeholder    ,  means index , useless  so we use “_” instead

	for _, item := range searchResult.Each(reflect.TypeOf(ptype)) {
		p := item.(Post) // cast to Post type . data type put in the back
