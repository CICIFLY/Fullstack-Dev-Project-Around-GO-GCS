...
import com.xzhou.jupiter.entity.ItemType;
...
public class ItemRecommender {
    //default limits for recommendation
        ...
        for (Game game : topGames) {
            ...
            for (Item item : items) {
                if (recommendedItems.size() == DEFAULT_TOTAL_RECOMMENDATION_LIMIT) {
                    break outerloop; //可以直接在达到条件后在这里return recommendedItems;　更习惯一点
                }
    ...
    // All items are related to the items previously favorited by the user. E.g., if a user favorited
    // some videos about game "Just Chatting", then it will return some other videos about the same game.
    private List<Item> recommendByFavoriteHistory(
        ...
        Map<String, Long> favoriteGameIdByCount = favoriteGameIds.parallelStream()
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));   //parallelStream 更快
    ...
    // Each key is corresponding to a list of Items objects, each item object is a recommended item based
    // on the previous favorite records by the user.
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            // entry.getKey ----> “videos”   entry.getValue---> gameIDs
    ...
    // Return a map of Item objects as the recommendation result. Keys of the map are [Stream, Video, Clip]. Each key is corresponding to a list of Items objects, each item object is a recommended item based on the top games currently on Twitch.
